...
def convert_amr_file(input_dir, input_file_name, output_dir):
    ...
    mp3_output_file_name = mp3_output_file_name[:-5]
    mp3_output_file_path = os.path.join(output_dir, mp3_output_file_name)
    print(mp3_output_file_path)
    ...
    subprocess.call(["ffmpeg", "-i", wav_output_file_path, mp3_output_file_path],
                    stdout=black_hole_file, stderr=black_hole_file)
...
def convert_silk_file(input_dir, input_file_name, decoder_file_path, output_dir):
    ...
    intermediate_file_1_path = os.path.join(
        output_dir, intermediate_file_1_name)
    ...
    intermediate_file_2_path = os.path.join(
        output_dir, intermediate_file_2_name)
    intermediate_file_2 = open(intermediate_file_2_path, 'wb')

    intermediate_file_3_name = input_file_name.replace(".aud", ".wav")
    intermediate_file_3_name = intermediate_file_3_name[:-5]
    intermediate_file_3_path = os.path.join(
        output_dir, intermediate_file_3_name)
    mp3_output_file_name = input_file_name.replace(".aud", ".mp3")
    mp3_output_file_name = mp3_output_file_name[:-5]
    mp3_output_file_path = os.path.join(output_dir, mp3_output_file_name)
    ...
    print("decoding to ", intermediate_file_2_path)
    subprocess.call([decoder_file_path, intermediate_file_1_path, intermediate_file_2_path],
                    stdout=black_hole_file, stderr=black_hole_file)

    print("converting to wav", intermediate_file_2_path,
          intermediate_file_3_path, " ".join(["ffmpeg", "-f", "s16le", "-ar", "24000",
                                              "-i", intermediate_file_2_path,
                                              intermediate_file_3_path]))
    ...
    print("converting to mp3", " ".join(["ffmpeg", "-i", intermediate_file_3_path,
                                        mp3_output_file_path]))
